import TRAC_COMS 
from TRAC_COMS import *
import time
import traceback
import math as m
import threading
from hawk_configurator import configuration
...
class INS(object):
    def __init__(self, COM='/dev/ttyUSB0',baud = 921600):
        ...
        self.HITL_ID = 0x000E
        self.FCNOFIG_ID = 0x001E
        self.SCONFIG_ID = 0x002E
    ...
    def send_heartbeat(self):
        ...
        if(self.Tx_ID == self.FCNOFIG_ID):
            self.data_conf.load_config("frame_params.yaml")
            for i in range(len(self.data_conf.conf)):
                def_id = message_id

                message_1 = np.array([self.data_conf.address[i], self.data_conf.weight[i]],dtype='int16')
                if(self.data_conf.weight[i] == 4):
                    X = np.array([self.data_conf.conf[i]], dtype='float32')
                else:
                    X = np.array([self.data_conf.conf[i]], dtype ='int16')
                message_2 = X.tobytes()
                message_2 = np.frombuffer(message_2, dtype = 'int16')
                
                def_id = np.concatenate((def_id, message_1, message_2), axis = 0)
                print("data ", self.data_conf.address[i], " = ", self.data_conf.conf[i])
                self.com.send(def_id)
                time.sleep(0.02)
                self.com.clear()
                self.com.clear()
                self.com.clear()
            return ## exit because I'm too lazy to figure out a clean way to implement it all together gosh
        ...
        try:
            ...
            if(num_bytes>=4):
                ...
                while(num_bytes<68 and time.time()-time_stamp<0.1):
                    # time.sleep(0.00001)
                    num_bytes = self.com.check_recv()
                if(time.time()-time_stamp >= 0.1 and self.got_first_message):
                    self.com.clear()
                    return
